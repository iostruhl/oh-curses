...
class GameBoard:
    ...
    def finish_trick(self):
        ...
        # calculate winner
        winner = max(self.in_play.values(), key=lambda x: Card.trick_value(x, self.led_suit, self.trump_suit))
...
if __name__ == "__main__":
    ...
    print("playing trick")
    g.play_card(g.players[0], g.hands["0"][0], lead = True)
    g.play_card(g.players[1], g.hands["1"][0])
    g.play_card(g.players[2], g.hands["2"][0])
    g.play_card(g.players[3], g.hands["3"][0])
    print("hands now are ", g.hands)
