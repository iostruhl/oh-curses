...
class ClientChannel(Channel):
    ...
    def Network_name(self, data):
        ...
        self.Send({'action': "server_name", 'name': self.name})
...
class OHServer(Server):
    ...
    def handle_ready(self):
        ...
        if (self.ready_count == 4):
            if (self.should_resume):
                self.resume_game(
                )  # SHOULD EVENTUALLY CALL RESUME, IDK WHY IT DOESN'T WORK RN
            else:
                self.start_game()
    ...
    def start_hand(self):
        ...
        self.send_all({
            'action': "broadcast_dealer",
            'dealer': self.gb.players[(self.hand_num + 2) % 4]
        })
        for player in self.gb.players:
            ...
            self.send_one(player, {
                ...
                "hand_dealt",
                'trump_card':
                self.gb.trump_card.to_array() if self.gb.trump_card else None,
        ...
        self.send_all({
            ...
            'actor': self.gb.players[(self.hand_num - 1) % 4],
    ...
    def handle_bid(self, player: str, bid: int):
        ...
        if (len(self.gb.bids) == 4):
            self.next_to_play_idx = (
                self.hand_num -
                1) % 4  # set to dealer, incremented by handle_play
            ...
            self.send_all({
                'action':
                "broadcast_current_actor",
                'actor':
                self.gb.players[((self.hand_num - 1) + len(self.gb.bids)) % 4],
                'hand_num':
                self.hand_num
            })
            self.send_one(
    ...
    def finish_trick(self):
        ...
        if (len(self.gb.hands[winner]) == 0):
            ...
            self.next_to_play_idx = self.gb.players.index(
                winner)  # start with the prev winner
            self.send_all({
                'action': "broadcast_current_actor",
                'actor': self.gb.players[self.next_to_play_idx],
                'hand_num': self.hand_num
