...
class GraphicsBoard:
    ...
    def hand_navigate(self, n):
        ...
        self.hand_windows[0][self.hand_position].mvwin((5*PADDING)+(2*C_HEIGHT)+(12*V_SPACING)+I_HEIGHT, (2*PADDING)+C_WIDTH+(self.hand_position*H_SPACING)+self.cols_offset)
        ...
        while not self.cb.is_playable(self.cb.hands[self.cb.active][self.hand_position]):
            if n == 0:
                ...
                self.hand_position = (self.hand_position + n) % len(self.cb.hands[self.cb.active])
        self.hand_windows[0][self.hand_position].mvwin((5*PADDING)+(2*C_HEIGHT)+(12*V_SPACING)+I_HEIGHT-2, (2*PADDING)+C_WIDTH+(self.hand_position*H_SPACING)+self.cols_offset)
    ...
    def draw_new_hand(self, hand_size: int):
        for i in range(len(self.cb.players)):
            if i == self.cb.active_position:
                self.hand_windows[0] = [curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+(2*C_HEIGHT)+(12*V_SPACING)+I_HEIGHT, (2*PADDING)+C_WIDTH+(j*H_SPACING)+self.cols_offset)
                                        for j in range(hand_size)]
                self.hand_panels[0] = [panel.new_panel(self.hand_windows[0][j])
                                       for j in range(hand_size)]
            elif i == (self.cb.active_position + 1) % 4:
                self.hand_windows[1] = [curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+C_HEIGHT+(j*V_SPACING), PADDING+self.cols_offset)
                                        for j in range(hand_size)]
                self.hand_panels[1] = [panel.new_panel(self.hand_windows[1][j])
                                       for j in range(hand_size)]
            elif i == (self.cb.active_position + 2) % 4:
                self.hand_windows[2] = [curses.newwin(C_HEIGHT, C_WIDTH, PADDING, (2*PADDING)+C_WIDTH+(j*H_SPACING)+self.cols_offset)
                                        for j in range(hand_size)]
                self.hand_panels[2] = [panel.new_panel(self.hand_windows[2][j])
                                       for j in range(hand_size)]
            elif i == (self.cb.active_position + 3) % 4:
                self.hand_windows[3] = [curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+C_HEIGHT+(j*V_SPACING), (3*PADDING)+(2*C_WIDTH)+(12*H_SPACING)+self.cols_offset)
                                        for j in range(hand_size)]
    ...
    def collapse_hand(self, player_position):
        ...
        if window_position == 0:
            self.hand_windows[window_position][self.hand_position].mvwin((5*PADDING)+(2*C_HEIGHT)+(12*V_SPACING)+I_HEIGHT, (2*PADDING)+C_WIDTH+(self.hand_position*H_SPACING)+self.cols_offset)
    ...
    def draw_new_info_window(self):
        for i in range(len(self.cb.players)):
            if i == self.cb.active_position:
                self.info_windows[0] = curses.newwin(I_HEIGHT, I_WIDTH, (3*PADDING)+(2*C_HEIGHT)+(12*V_SPACING), (1*PADDING)+C_WIDTH+(6*H_SPACING)+self.cols_offset)
                ...
                self.info_windows[1] = curses.newwin(I_HEIGHT, I_WIDTH, C_HEIGHT+(12*V_SPACING), (2*PADDING)+C_WIDTH+self.cols_offset)
                ...
                self.info_windows[2] = curses.newwin(I_HEIGHT, I_WIDTH, C_HEIGHT-PADDING, (1*PADDING)+C_WIDTH+(6*H_SPACING)+self.cols_offset)
                ...
                self.info_windows[3] = curses.newwin(I_HEIGHT, I_WIDTH, C_HEIGHT+(12*V_SPACING), C_WIDTH+(12*H_SPACING)+self.cols_offset)
    ...
    def play_card(self, player):
        ...
        if player_position == 0:
            self.played_windows[0] = curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+C_HEIGHT+(12*V_SPACING), (2*PADDING)+C_WIDTH+(6*H_SPACING)+self.cols_offset)
        elif player_position == 1:
            self.cb.hands[player].pop()
            self.played_windows[1] = curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+C_HEIGHT+(6*V_SPACING), (2*PADDING)+C_WIDTH+(3*H_SPACING)+self.cols_offset)
            self.collapse_hand(self.cb.players.index(player))
        elif player_position == 2:
            self.cb.hands[player].pop()
            self.played_windows[2] = curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+C_HEIGHT, (2*PADDING)+C_WIDTH+(6*H_SPACING)+self.cols_offset)
            self.collapse_hand(self.cb.players.index(player))
            self.refresh_info_window(player)
        elif player_position == 3:
            self.cb.hands[player].pop()
            self.played_windows[3] = curses.newwin(C_HEIGHT, C_WIDTH, (5*PADDING)+C_HEIGHT+(6*V_SPACING), (2*PADDING)+C_WIDTH+(9*H_SPACING)+self.cols_offset)
            self.collapse_hand(self.cb.players.index(player))
    ...
    def draw_bids(self, hand_size, is_dealer):
        self.bid_windows = [curses.newwin(B_HEIGHT, B_WIDTH, (3*PADDING)+(3*C_HEIGHT)+(12*V_SPACING)+I_HEIGHT, (2*PADDING)+C_WIDTH+(i*B_WIDTH)+self.cols_offset)
    ...
    def draw_trump(self):
        if self.cb.trump_card:
            self.trump_window = curses.newwin(C_HEIGHT, C_WIDTH, PADDING, PADDING+self.cols_offset)
