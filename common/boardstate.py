...
class GameBoard:
    def __init__(self, players=["Isaac", "Alex", "Ben", "Owen"]):
        ...
        self.deck = [
            Card(rank, suit) for suit in Card.suit_ascii
    ...
    def play_card(self, player: str, card: Card, lead=False):
        ...
        assert (player not in self.in_play
                ), "%s has already played a card" % player
        print("Playing card", card, "from player", player,
              "(trick lead)" if lead else "")
        if (lead):
    ...
    def finish_trick(self) -> str:
        ...
        for player in self.in_play:
            assert (self.in_play[player] is
                    not None), "Finish trick: not everyone has played a card"

        # calculate winner
        winner = max(self.in_play,
                     key=lambda x: Card.trick_value(
                         self.in_play[x], self.led_suit, self.trump_card.suit
    ...
    def update_scores(self) -> dict:
        handscores = dict()
        for player in self.players:
            if len(self.cards_taken[player]
                   ) / 4 == self.bids[player]:  # made hand
                handscores[player] = int(10 + (self.bids[player]**2))
            else:
                diff = abs((len(self.cards_taken[player]) / 4) -
                           self.bids[player])
    ...
    def assert_hand_done(self):
        for player in self.hands:
            assert (self.hands[player] == []
                    ), "Hand done: not everyone's hands are empty"
    ...
    def get_hand(self, dealt_hand):
        for player in self.hands.keys():
            if player == self.active:
                self.hands[player] = [
                    Card(card[0], card[1]) for card in dealt_hand
                ]
